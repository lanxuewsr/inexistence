...
[[ $DeBUG == 1 ]] && { iUser=lanxuewsr ; aptsources=No ; MAXCPUS=$(nproc) ; }
...
function _intro() {
    ...
    wget --no-check-certificate -t1 -T5 -qO- https://raw.githubusercontent.com/lanxuewsr/inexistence/files/logo/inexistence.logo.1
...
#    apt_sources_add
    ...
    git clone --depth=1 -b $iBranch https://github.com/lanxuewsr/inexistence /etc/inexistence >> "$OutputLOG" 2>&1
...
EOF
    ...
    if [[ ! -f /etc/abox/app/BDinfoCli.0.7.3/BDInfo.exe ]]; then
        ...
        svn co https://github.com/lanxuewsr/bluray/trunk/tools/BDinfoCli.0.7.3  >> "$OutputLOG" 2>&1
        mv -f BDinfoCli.0.7.3 BDinfoCli                                        >> "$OutputLOG" 2>&1
    fi

    if [[ ! -f /etc/abox/app/bdinfocli.exe ]]; then
        wget https://github.com/lanxuewsr/bluray/raw/master/tools/bdinfocli.exe -O -nv -N /etc/abox/app/bdinfocli.exe >> "$OutputLOG" 2>&1
    fi
